...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 16 10:23:26 2018
...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 14 14:36:07 2018
            ...
            if g[s_x+action[0]][s_y+action[1]]>g[s_x][s_y]+1:
                ...
                for count,open in enumerate(openlist): 
                    if open[1][0]==s_x+action[0] and open[1][1]==s_y+action[1]: 
                        ...
                        openlist.pop()
                        heapify(openlist)
                if h[s_x+action[0]][s_y+action[1]]==0:
                    h[s_x+action[0]][s_y+action[1]]=heuristic((s_x+action[0],s_y+action[1]))
                f=g[s_x+action[0]][s_y+action[1]]+h[s_x+action[0]][s_y+action[1]]
                heappush(openlist,(f,(s_x+action[0],s_y+action[1])))
    
    for close in closedlist: 
        #print('closedlis',close)
        h[close[0]][close[1]]=g[target_x][target_y]-g[close[0]][close[1]]
...
for z in range(1,51):               
    ...
    while start_x!=target_x or start_y!=target_y:
        ...
        closedlist=set()
        openlist=[]
        h[start_x][start_y]=heuristic((start_x,start_y))
        f=g[start_x][start_y]+h[start_x][start_y]
        heappush(openlist,(f,(start_x,start_y)))
        ComputePath(counter)
        print('in main')
        if len(openlist)==0:
    ...
    file= open("forwardvsadaptive.txt","a") 
    #file.write("mazeno\tForward\tAdaptive\n")
