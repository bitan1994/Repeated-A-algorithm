...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 15 09:25:21 2018
...
def ComputePath2(targets_x,targets_y):
    print('in compute path')
    global counter
    global nexpanded2
    while  len(openlist)!=0 and g[targets_x][targets_y]>min(openlist)[0] :
        s=getfromOpenlist()
        #print(s)
        s_x=s[0]
        s_y=s[1]
        ...
        for action in actionlists:
            ...
            if g[s_x+action[0]][s_y+action[1]]>g[s_x][s_y]+1:
                ...
                f=g[s_x+action[0]][s_y+action[1]]+heuristic2((s_x+action[0],s_y+action[1]),(targets_x,targets_y))
                heappush(openlist,(f,(s_x+action[0],s_y+action[1])))     
...
for z in range(25,26):            
    ...
    fixedtarget_x=target_x
    fixedtarget_y=target_y
    ...
    #target_x=13#fixedstart_x
    #target_y=96#fixedstart_y
    print('start at',start_x,start_y)
    print('target at',target_x,target_y)
    ...
    while start_x!=target_x or start_y!=target_y:
        ...
        search[target_x][target_y]=counter
        g[start_x][start_y]=MAX_INT
        search[start_x][start_y]=counter
        closedlist=set()
        openlist=[]
        f=g[target_x][target_y]+heuristic2((target_x,target_y),(start_x,start_y))
        heappush(openlist,(f,(target_x,target_y)))
        ComputePath2(start_x,start_y)
        print('in main')
        if len(openlist)==0:
            print("cant reach target")
            sys.exit()
            break
        path=backtrace(tree,(target_x,target_y),(start_x,start_y))
        path.reverse()
        i=1
        print(path)
        while current_x!=target_x or current_y!=target_y  : #and len(actions(current_x,current_y))!=0:
            ...
            current_x=path[i][0]
            current_y=path[i][1]
            showpath.append((current_x,current_y))
            i=i+1
            print(current_x,current_y)
            #print(current_x!=target_x and current_y!=target_y and i<len(path))
        if(current_x!=start_x or current_y!=start_y):
    ...
    file= open("compareForwardBackward.txt","a") 
    #file.write("mazeno Forward Backward\n")
    file.write("%d\t%d\t%d\n" % (z,nexpanded,nexpanded2))  
    file.close()   
    for p in showpath:
        if blocked[p[0]][p[1]]==1:
            print('WRONG')
            break
        blocked[p[0]][p[1]]=2
